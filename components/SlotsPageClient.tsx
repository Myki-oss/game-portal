...
export default function SlotsPageClient() {
  ...
  const [paginatedGames, setPaginatedGames] = useState<Game[]>([]); // Nama fungsi yang benar

  const allGames = getGamesByCategory("slots");
  const providers = getProviders();
  const gamesPerPage = 24;
  const currentPage = Number(searchParams.get("page")) || 1;

  // --- Efek untuk Filter dan Sort ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setPaginatedGames(paginated); // <-- TYPO SUDAH DIPERBAIKI DI SINI
  ...
  return (
    <div className="py-12">
      <div className="container mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white md:text-4xl">Game Slot Online</h1>
          <div className="mt-4 max-w-3xl text-neutral-300">
            <p>
              Rasakan sensasi kemenangan jackpot di ribuan game slot online terlengkap dari provider
              ternama dunia. Setiap game menawarkan tema unik, fitur bonus menarik, dan tentu saja,
              peluang menang besar dengan RTP (Return to Player) yang kompetitif.
            </p>
            <p className="mt-4">
              Provider seperti Pragmatic Play, PG Soft, dan Habanero terus merilis game-game
              inovatif dengan grafis memukau dan mekanisme permainan yang seru. Nikmati fitur
              seperti pembelian bonus (feature buy), putaran gratis dengan pengali tinggi, dan
              jackpot progresif yang bisa mengubah hidup Anda dalam sekejap.
            </p>
          </div>
        </div>
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center">
            <nav className="flex items-center space-x-1">
              {currentPage > 1 && (
                <button
                  onClick={() => router.push(`/slots?page=${currentPage - 1}`)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => router.push(`/slots?page=${page}`)}
                  className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                    page === currentPage
                      ? "bg-orange-600 text-white"
                      : "bg-neutral-800 text-white hover:bg-neutral-700"
