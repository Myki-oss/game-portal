...
export default function RacePageClient() {
  ...
  const allGames = getGamesByCategory("race"); // Ganti dengan "race"
  ...
  return (
    <div className="py-12">
      <div className="container mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white md:text-4xl">Taruhan Balapan Online</h1>
          <div className="mt-4 max-w-3xl text-neutral-300">
            <p>
              Saksikan dan pasang taruhan pada balapan paling seru di dunia, dari pacuan kuda klasik hingga kejuaraan mobil Formula 1. Dapatkan akses ke odds kompetitif, streaming langsung, dan statistik mendalam untuk membantu Anda membuat keputusan taruhan terbaik. Rasakan ketegangan setiap tikungan dan finis di garis finish.
            </p>
            <p className="mt-4">
              Platform kami menawarkan berbagai pasar taruhan untuk balapan kuda, greyhound, dan motorsport dari seluruh dunia. Baik Anda penggemar balapan tradisional atau lebih suka aksi virtual yang cepat, kami memiliki semuanya. Nikmati pengalaman taruhan yang aman, adil, dan penuh kegembiraan hanya di sini.
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            {providers.map((provider) => (
              <button
                key={provider.id}
                onClick={() => router.push(`/race?provider=${encodeURIComponent(provider.name)}`)}
                className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center">
            <nav className="flex items-center space-x-1">
              {currentPage > 1 && (
                <button
                  onClick={() => router.push(`/race?page=${currentPage - 1}`)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => router.push(`/race?page=${page}`)}
                  className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                    page === currentPage
                      ? "bg-orange-600 text-white"
                      : "bg-neutral-800 text-white hover:bg-neutral-700"
