...
export default function LiveCasinoPageClient() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [sortedGames, setSortedGames] = useState<Game[]>([]);
  const [paginatedGames, setPaginatedGames] = useState<Game[]>([]);

  // Ganti "slots" dengan "live-casino"
  const allGames = getGamesByCategory("live-casino");
  ...
  return (
    <div className="py-12">
      <div className="container mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white md:text-4xl">Live Casino Online</h1>
          <div className="mt-4 max-w-3xl text-neutral-300">
            <p>
              Rasakan pengalaman kasino sungguhan langsung dari layar Anda dengan Live Casino kami. Mainkan permainan klasik seperti Baccarat, Roulette, dan Blackjack yang dipandu oleh dealer profesional secara real-time. Streaming video berkualitas HD dan interaksi langsung menciptakan atmosfer kasino mewah di mana pun Anda berada.
            </p>
            <p className="mt-4">
              Nikmati berbagai pilihan meja dengan taruhan yang sesuai untuk semua pemain, dari pemula hingga high roller. Provider ternama seperti Evolution Gaming dan Pragmatic Play menyajikan inovasi terbaru dalam game live, termasuk game show interaktif seperti Crazy Time dan Monopoly Live yang menawarkan keseruan dan peluang menang tak terbatas.
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            {providers.map((provider) => (
              <button
                key={provider.id}
                onClick={() => router.push(`/live-casino?provider=${encodeURIComponent(provider.name)}`)}
                className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                params.delete("page");
                router.push(`/live-casino?${params.toString()}`);
              }}
            >
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center">
            <nav className="flex items-center space-x-1">
              {currentPage > 1 && (
                <button
                  onClick={() => router.push(`/live-casino?page=${currentPage - 1}`)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => router.push(`/live-casino?page=${page}`)}
                  className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                    page === currentPage
                      ? "bg-orange-600 text-white"
                      : "bg-neutral-800 text-white hover:bg-neutral-700"
