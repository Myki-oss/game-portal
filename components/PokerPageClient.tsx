...
export default function PokerPageClient() {
  ...
  const allGames = getGamesByCategory("poker"); // Ganti dengan "poker"
  ...
  return (
    <div className="py-12">
      <div className="container mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white md:text-4xl">Game Poker Online</h1>
          <div className="mt-4 max-w-3xl text-neutral-300">
            <p>
              Uji kemampuan dan strategi Anda di meja poker online terbaik. Bergabunglah dengan ribuan pemain dalam berbagai varian poker, dari Texas Hold'em yang populer hingga Capsa Susun yang menantang. Nikmati pengalaman bermain yang adil dan aman dengan sistem keamanan terkini dan dealer profesional.
            </p>
            <p className="mt-4">
              Baik Anda seorang pemula yang ingin belajar atau pemain berpengalaman yang mencari tantangan, platform kami menawarkan meja dengan taruhan yang sesuai untuk semua level. Ikuti turnamen reguler dengan hadiah besar atau mainkan permainan kasual untuk melatih skill Anda. Jadilah juara dan raih kemenangan Anda di meja poker kami!
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            {providers.map((provider) => (
              <button
                key={provider.id}
                onClick={() => router.push(`/poker?provider=${encodeURIComponent(provider.name)}`)}
                className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                params.delete("page");
                router.push(`/poker?${params.toString()}`);
              }}
            >
        ...
        {totalPages > 1 && (
          <div className="mt-12 flex justify-center">
            <nav className="flex items-center space-x-1">
              {currentPage > 1 && (
                <button
                  onClick={() => router.push(`/poker?page=${currentPage - 1}`)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => router.push(`/poker?page=${page}`)}
                  className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
                    page === currentPage
                      ? "bg-orange-600 text-white"
                      : "bg-neutral-800 text-white hover:bg-neutral-700"
